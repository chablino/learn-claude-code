...
class TeammateManager:
    ...
    def _teammate_loop(self, name: str, role: str, prompt: str):
        ...
        should_exit = False
        time.sleep(3)  # avoid busy loop if something goes wrong
        for _ in range(50):
            ...
            if response.stop_reason != "tool_use":
                # # 大模型没有调用工具，说明当前无事可做，进入“待机监听”模式
                # timeout_seconds = 60  # 设置最大耐心值（60秒内没信就下班）
                # poll_interval = 2     # 每 2 秒悄悄看一次信箱（不耗费大模型 API）
                # elapsed = 0
                # new_messages_found = False
                
                # while elapsed < timeout_seconds and not should_exit:
                ...
                #     # 带着新消息继续外层大循环，重新唤醒大模型处理请求
                #     continue 
                # else:
                #     # 苦等 60 秒依然没有信，或者收到系统强行退出信号
                #     print(f"  [{name}] Timeout waiting for new messages. Going idle.")
                #     break # 真正跳出外层循环，准备收尾
                break
            results = []
...
TOOL_HANDLERS = {
    ...
    "shutdown_response": lambda **kw: _check_shutdown_status(
        kw.get("request_id", "")
    ),
